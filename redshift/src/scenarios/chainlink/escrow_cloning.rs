...
impl Scenario for EscrowCloning {
    ...
    async fn run(&self, base: &BaseCtx, er: &ErCtx) -> Result<ScenarioReport> {
        ...
        check::poll(
            "the ER clones the spender escrow pda",
            CLONE_TIMEOUT,
            || async {
        ...
        .await?;
        check::poll(
            "the executed ER transfer lands on the receiver",
            CLONE_TIMEOUT,
            || async {
                matches!(
                    er.api().get_balance(&receiver.pubkey()).await,
        ...
        check_eq!(
            spender_escrow_after.lamports,
            spender_escrow_before.lamports,
            "an EXECUTED er transfer must not move escrow lamports"
        )?;
        check_eq!(
            spender_escrow_after.data,
            spender_escrow_before.data,
