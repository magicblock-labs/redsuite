...
use pubkey::Pubkey;
use redline::program::{instruction::build, DELEGATION_PROGRAM_ID};
use redsuite_core::{
    api::custom_error_code,
    assert::poll_until,
    prep, receipt, report, run_scenario,
    ...
    async fn run(&self, base: &BaseCtx, er: &ErCtx) -> Result<ScenarioReport> {
        ...
        if profile.probes {
            ...
            // the 11th sponsored commit on one account is rejected on-chain
            {
                ...
                let probe_pdas = redline::init_delegated_accounts(
                    base,
                    &probe_payer,
                    1,
                    redline::ACCOUNT_SPACE,
                ...
                poll_until(CLONE_TIMEOUT, || async {
                    matches!(er.account(&probe_account).await, Ok(Some(acc)) if acc.data.len() == redline::ACCOUNT_SPACE as usize)
                })
                .await;
...
async fn commit_width_envelope() {
    run_scenario(CommitWidthEnvelope).await;
}
