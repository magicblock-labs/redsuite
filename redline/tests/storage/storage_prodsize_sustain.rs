    ...
    async fn run(&self, base: &BaseCtx, er: &ErCtx) -> Result<ScenarioReport> {
        ...
        let prep_payers =
            prep::funded_payers(base, profile.payers, PREP_PAYER_LAMPORTS)
                .await?;
        let pool = redline::init_delegated_accounts_batched(
            base,
            &prep_payers,
            profile.accounts,
        ...
        for (cell_name, superblock_slots) in cells_spec {
            ...
            for pda in &pool {
                poll_until(CLONE_TIMEOUT, || async {
                    matches!(cell_er.account(pda).await, Ok(Some(acc)) if acc.data.len() == redline::ACCOUNT_SPACE as usize)
                })
                .await;
            }
...
async fn storage_prodsize_sustain() {
    run_scenario(StorageProdsizeSustain).await;
}
