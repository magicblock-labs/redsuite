...
async fn burst_cell(
    ...
    if prewarm_deps {
        for sender in &senders {
            let warm_ix =
                redline::program::instruction::build::read_accounts_data(
                    0,
                    &[prewarm_account],
                );
    ...
    let request = {
        ...
        move |id: u64| {
            ...
            let ix = redline::program::instruction::build::read_accounts_data(
                id,
                &[account],
            );
            let slow = slow.clone();
    ...
    async fn run(&self, base: &BaseCtx, er: &ErCtx) -> Result<ScenarioReport> {
        ...
        let prep_payers =
            prep::funded_payers(base, profile.prep_payers, PREP_PAYER_LAMPORTS)
                .await?;
        let all_accounts = redline::init_accounts_batched(
            base,
            &prep_payers,
...
async fn cold_hydration_tail() {
    run_scenario(ColdHydrationTail).await;
}
